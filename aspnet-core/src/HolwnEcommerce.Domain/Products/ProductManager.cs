    ...
    public class ProductManager : DomainService
    {
            ...
            string thumbnailPicture, double sellPrice)
        {
            ...
            if(await _productRepository.AnyAsync(x=>x.Code == code)) {
                throw new UserFriendlyException("Mã sản phẩm đã tồn tại", HolwnEcommerceDomainErrorCodes.ProductCodeAlreadyExists);
            }
            if(await _productRepository.AnyAsync(x=>x.SKU == sKU)) {
                throw new UserFriendlyException("Mã SKU sản phẩm đã tồn tại", HolwnEcommerceDomainErrorCodes.ProductSKUAlreadyExists);
            }
