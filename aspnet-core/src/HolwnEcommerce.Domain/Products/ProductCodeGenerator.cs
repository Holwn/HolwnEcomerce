...
namespace HolwnEcommerce.Products
{
    public class ProductCodeGenerator : ITransientDependency
    {
        ...
        {
            string newCode;
            var identitySetting = await _identitySettingRepository.FindAsync(HolwnEcommerceConsts.ProductIdentitySettingId);
            if (identitySetting == null)
            {
                identitySetting = await _identitySettingRepository.InsertAsync(new IdentitySetting(HolwnEcommerceConsts.ProductIdentitySettingId, "Sản phẩm", HolwnEcommerceConsts.ProductIdentitySettingPrefix, 1, 1));
